...
model = TransformerModel(input_dim, output_dim,
                         hidden_dim, layers, 4, dropout_rate)
...
onedimensionaltesttarget = test_target.view(-1)
...
def train_model(model, criterion, optimizer, num_epochs=100):
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        if (epoch + 1) % epochs_step == 0:
            ...
            test_loss = unratio_criterion(outputs,
                                          onedimensionaltesttarget)
            test_losses.append(test_loss.item())

            outputs = outputs > 0.5

            sensitivity = torch.sum(outputs[onedimensionaltesttarget == 1] == 1).item(
            ) / torch.sum(onedimensionaltesttarget == 1).item()
            sensitivities.append(sensitivity)

            sppecificity = torch.sum(outputs[onedimensionaltesttarget == 0] == 0).item(
            ) / torch.sum(onedimensionaltesttarget == 0).item()
            ...
            if (epoch + 1) % (epochs_step) == 0:
                ...
                torch.save(model.state_dict(),
                           'pths/Trans-epoch-{}-acc-{:.4f}-tloss-{:.4f}-sens-{:.4f}.pth'.format(epoch+1, test_loss.item()*100, train_loss*100, sensitivity*100))
...
if train == True:
    ...
    plt.savefig(
        'outputs/Tr-{:04}-{:02.4f}.png'.format(num_epochs, test_losses[-1]*100))
    plt.show()

    torch.save(model.state_dict(), 'pths/Tr-epoch-{}-acc-{:.4f}-sens-{:.4f}.pth'.format(
